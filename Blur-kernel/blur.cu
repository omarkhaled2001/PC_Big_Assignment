...
__global__ void gaussianBlurKernel(unsigned char* d_in, unsigned char* d_out, int width, int height, int channels, float* d_kernel, int kernelRadius) {
...
int main(int argc, char** argv) {
    ...
    int kernelRadius = 10;  // Increased from 2 to 5
    float sigma = 5.0f;    // Increased from 1.0f to 2.0f
    ...
    gaussianBlurKernel<<<dimGrid, dimBlock>>>(d_in, d_out, width, height, channels, d_kernel, kernelRadius);
